...
import com.tomatoLCJC.usbutil.DeviceInfo;
...
public class DeviceDetectionRecordDao {
    ...
    public void initChannelDetectionRecordTable(){
        ...
        if (rowCount == 0) {
            ...
            chanDeteRecordBean.setDeviceID(1);
            chanDeteRecordBean.setDeviceName("设备1");
    ...
    public void updateDetectionData(DeviceDetectionRecordBean bean ){
        ...
        SimpleDateFormat sdf=new SimpleDateFormat("yyyyMMddHHmmssSS");
        String detectionTime = sdf.format(date);
        bean.setDeviceID(SystemParameter.getInstance().getDeviceID());         //设备ID
        String info=DeviceInfoDao.getInstance().query(SystemParameter.getInstance().getDeviceID()).getDeviceName();
        bean.setDeviceName(info);         //设备名称
    ...
    /**********************************************************************************************
     * 方法名  :  importDetectionData
     * 功能描述: 更新校准数据第二种方法,将状态为0的那一行数据取出置为历史,选定的id的记录为此时的记录
     **********************************************************************************************/
    public void importDetectionData(long id ){
        //更新上一条数据
        DeviceDetectionRecordBean cDR = getStatusIsZeroBean();
        long preId =cDR.getId();
        if (id==preId){
            return;
        }
        cDR.setStatus(1);//将上一条置为历史
    ...
    public List<DeviceDetectionRecordBean> queryAllByID(){
        long id = SystemParameter.getInstance().getDeviceID();
        return DataSupport
                .where("deviceID == ?", String.valueOf(id))
                .find(DeviceDetectionRecordBean.class);
    }
