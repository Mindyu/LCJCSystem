...
public class ChangeButtonNumsUtil {
    ...
    public ChangeButtonNumsUtil(int channleCount, LinearLayout channelGroupLayout, Fragment fragment, Context context) {
    ...
    public void initChannelButtons() {
        ...
        for (int i = 0; i < channleCountTemp; i++) {
            ...
            imageButton.setBackgroundColor(ContextCompat.getColor(context, R.color.channelBtnBack));
...
//                            GlobalParameter.getInstance().setLineRecover(flag);
                            if (fragment instanceof FragmentDataMeasure){
                                FragmentDataMeasure.getInstance().chartView.setLineRecover(flag);
                                FragmentDataMeasure.getInstance().scanView.setLineRecover(flag);
                            }else {
                                SecondFragmentDataMeasure.getInstance().chartView_1.setLineRecover(flag);
                                SecondFragmentDataMeasure.getInstance().chartView_2.setLineRecover(flag);
                            }
                        } else {
...
//                            GlobalParameter.getInstance().setLineTransparent(flag);
                            if (fragment instanceof FragmentDataMeasure){
                                FragmentDataMeasure.getInstance().chartView.setLineRemove(flag);
                                FragmentDataMeasure.getInstance().scanView.setLineRemove(flag);
                            }else {
                                SecondFragmentDataMeasure.getInstance().chartView_1.setLineRemove(flag);
                                SecondFragmentDataMeasure.getInstance().chartView_2.setLineRemove(flag);
                            }
                        }
                    } else {
                        showDialog();
    ...
    private void showDialog() {
        ...
        for (int i = 0; i < layoutNum; i++) {
            for (int j = 0; j < buttons[i]; j++) {
                ...
                imageButton.setBackgroundColor(ContextCompat.getColor(context, R.color.channelBtnBackInDialog));
                LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f);
                imageButton.setLayoutParams(lp); //设置每个按钮权重为1
